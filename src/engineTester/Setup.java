...
public class Setup
{
    ...
    public void Update_Health_Gui() {
        GuiTexture gui3 = new GuiTexture(loader.loadTexture("health/h" + health_count), new Vector2f(0.8f, 0.9f), new Vector2f(0.2f, 0.2f));
    ...
    public Setup() {
        ...
        entities.add(player);
        Enemy enemy = new Enemy(playerModel, new Vector3f(px, py, pz), 0, 2, 0, 0.6f);
        entities.add(enemy);
        enemies.add(enemy);
        ...
        while (!Display.isCloseRequested()) {
            ...
            if(enemy.getHealth() > 0){
                enemy.moving(player.getPosition(), player);
            }
            else{
                if(entities.contains(enemy)){entities.remove(enemy); enemies.remove(enemy);}
            }

            if(player_hp > player.getHealth()){
                Update_Health_Gui();
            }
