...
public class Camera {
	...
	public void update() {
		...
		// Turn the camera left when Q is pressed
		if (Input.isKeyDown(GLFW.GLFW_KEY_Q)) rotation = Vector3f.add(rotation, new Vector3f(0, -mouseSensitivity * 10, 0));
		// Turn the camera right when E is pressed
		if (Input.isKeyDown(GLFW.GLFW_KEY_E)) rotation = Vector3f.add(rotation, new Vector3f(0, mouseSensitivity * 10, 0));
